...
radius = 0.125
if h5_save:
    dx = 1./512 # spatial step in y direction also equal to dt
else:
    dx = 1./128
la = 1. # velocity of the scheme
rhoo = 1.
uo = 0.05
mu = 5.e-5 #change mu to change Re
zeta = 10*mu
...
if h5_save:
    ...
    while sol.t < Tf:  #every big loop increments dt by 0.125 s, number of frames = 500/0.125 = 4000
        for k in range(64):
            sol.one_time_step()
        im += 1 #every iteration has 64 subiterations
        printProgress(im, l, prefix='Progress:', suffix='Complete', barLength=50)
        save(sol.domain.mpi_topo, x, y, sol.m, im) #h5 saved after every 64 iterations
else:
